defmodule PhilomenaWeb.Admin.ReportControllerTest do
  ...
  describe "create report" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.admin_report_path(conn, :show, id)

      conn = get(conn, Routes.admin_report_path(conn, :show, id))
  ...
  describe "update report" do
    ...
    test "redirects when data is valid", %{conn: conn, report: report} do
      ...
      assert redirected_to(conn) == Routes.admin_report_path(conn, :show, report)
  ...
  describe "delete report" do
    ...
    test "deletes chosen report", %{conn: conn, report: report} do
      conn = delete(conn, Routes.admin_report_path(conn, :delete, report))
      assert redirected_to(conn) == Routes.admin_report_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.admin_report_path(conn, :show, report))
      end
    end
  end
