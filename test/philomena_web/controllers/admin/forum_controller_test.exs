defmodule PhilomenaWeb.Admin.ForumControllerTest do
  ...
  describe "new forum" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.admin_forum_path(conn, :new))
  ...
  describe "create forum" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.admin_forum_path(conn, :create), forum: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.admin_forum_path(conn, :show, id)

      conn = get(conn, Routes.admin_forum_path(conn, :show, id))
      assert html_response(conn, 200) =~ "Show Forum"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.admin_forum_path(conn, :create), forum: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Forum"
    end
  end
  ...
  describe "update forum" do
    ...
    test "redirects when data is valid", %{conn: conn, forum: forum} do
      conn = put(conn, Routes.admin_forum_path(conn, :update, forum), forum: @update_attrs)
      assert redirected_to(conn) == Routes.admin_forum_path(conn, :show, forum)

      conn = get(conn, Routes.admin_forum_path(conn, :show, forum))
      assert html_response(conn, 200)
    end

    test "renders errors when data is invalid", %{conn: conn, forum: forum} do
      conn = put(conn, Routes.admin_forum_path(conn, :update, forum), forum: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Forum"
    end
  end
  ...
  describe "delete forum" do
    ...
    test "deletes chosen forum", %{conn: conn, forum: forum} do
      conn = delete(conn, Routes.admin_forum_path(conn, :delete, forum))
      assert redirected_to(conn) == Routes.admin_forum_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.admin_forum_path(conn, :show, forum))
      end
    end
  end
