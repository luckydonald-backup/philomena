defmodule Philomena.Images.Policy do
  ...
  def authorize(:update, user, image) do
    (image.hidden_from_users == false and image.user_id == user.id) or
      image_moderator?(user) or
      dupe_moderator?(user)
  end

  def authorize(:update_metadata, user, image),
    do: authorize(:read, user, image) and image.tag_editing_allowed
